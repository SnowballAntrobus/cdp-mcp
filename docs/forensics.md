...
## P6 — Phase 6 findings (2026-07)

**P6-1 — aarch64 `char` is unsigned: every CDP textfile parser loops forever, refusing all text inputs.** Both wave-1 curation agents independently hit it: on Linux aarch64 (this sandbox), EVERY textfile input — mixfiles, breakpoint files, notedata, sequence files — refuses with `... is not a valid CDP file`. Root cause: CDP's text readers compare `(char)fgetc() != EOF` (cdparse.c and friends); with the unsigned-char ABI default the comparison can never be true, so the reader runs off the end and the parse fails. macOS (x86_64/arm64 with Apple's signed-char default) and Linux x86_64 are unaffected — an ABI landmine, not a source regression. Fix: rebuild ALL binaries with `-fsigned-char`; the per-subdir CMakeLists clobber `CMAKE_C_FLAGS`, so the flag must be injected into each (the top-level `-DCMAKE_C_FLAGS` alone is insufficient) — `scripts/build_cdp8_linux.sh` now does both. Audio-path renders verified byte-identical across the rebuild, so all pre-rebuild curation findings stand. Source: `docs/curation/tranche12_submix_depth.md`, `tranche13_envelope.md`; `scripts/build_cdp8_linux.sh`.
