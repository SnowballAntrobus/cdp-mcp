...
> Wave-2 bug finds (newdelay feedback-±1 hang, submix mix overload WRAP,
> quirk unipolar zero-frame success) are recorded in
> `docs/curation/tranche7*/tranche8*` transcripts and the affected
> entries' `known_issues`; promotion into this file is queued for the
> Phase 5 handoff. P5-1 below is recorded immediately because it is a
> *platform-vintage* divergence that manual QA will keep hitting.
